...
```{r eval=FALSE}
install.packages(c("RSQLite", "gdata", "reshape2", "DBI"))
...
We begin by setting the R working directory to the location of the top level peat collapse data folder. The following code chunk sets the `R` "working directory":
...
limspath = file.path("Raw", "lab"), ppath = file.path("Raw", "lab",
"phosphorus"), sulfpath = file.path("Raw", "lab"), tofile = TRUE)
```

If the `tofile` parameter is set to `TRUE`, a summary file will be saved to the current working directory under the name `fieldall.csv`. This file must be manually appended with a version number before old versions can be moved to the `Archive` folder.
...
If the `tofile` parameter is set to `TRUE`, a summary file will be saved to the current working directory under the name `mesoall_soilplant.csv` or `mesoall_soilonly.csv`. These files must be manually appended with a version number before old versions can be moved to the `Archive` folder.
